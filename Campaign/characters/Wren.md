# Wren

**Race:** Human
**Class & Level:** Sorcerer 16 (Wild Magic)
**Background:** Criminal (self-taught survivor, Ravensmoor's fairground district)
**Alignment:** Chaotic Good

## Ability Scores
| STR | DEX | CON | INT | WIS | CHA |
|-----|-----|-----|-----|-----|-----|
| 8 (−1) | 14 (+2) | 16 (+3) | 10 (+0) | 12 (+1) | 20 (+5) |

## Combat
- **AC:** 15 (Mage Armor: 13 + DEX)
- **HP:** 82 / 82
- **Hit Dice:** 16d6
- **Speed:** 30 ft
- **Passive Perception:** 11
- **Proficiency bonus:** +5 (character level 15)

### Attacks
Not a melee fighter — relies entirely on spellcasting and Wild Magic. Carries a dagger for emergencies only.

| | To Hit | Damage |
|---|---|---|
| Dagger (finesse) | +1 | 1d4−1 piercing |
| Fire Bolt (cantrip) | +10 | 4d10 fire |
| Chaos Bolt (1st, upcasts) | +10 spell attack | 2d8+1d6 damage (see Features) |

**Spell save DC 18, spell attack +10.**

## Proficiencies & Skills
- **Saving throws:** Constitution +8, Charisma +10
- **Skills:** Insight +6, Persuasion +10, Deception +10, Stealth +7
- **Languages:** Common, Infernal (picked up on the street, never formally taught)
- **Tools:** none
- **Weapons:** daggers, darts, slings, quarterstaffs, light crossbows
- **Armor:** none (relies on Mage Armor)

## Features & Traits

**Sorcerer**
- **Spellcasting:** Charisma-based. Spell save DC 18, spell attack +10. Knows spells (doesn't prepare) — 6 cantrips, 15 spells known. Slots: 4 (1st), 3 (2nd), 3 (3rd), 3 (4th), 2 (5th), 1 (6th), 1 (7th).
- **Font of Magic:** 15 sorcery points. Can convert sorcery points to spell slots and vice versa (Flexible Casting).
- **Metamagic (3 known):** *Quickened Spell* (2 pts — cast a normal-action spell as a bonus action), *Twinned Spell* (varies — target a second creature with a single-target spell), *Careful Spell* (1 pt — chosen creatures automatically succeed on the spell's save).

**Wild Magic (Sorcerous Origin)**
- **Wild Magic Surge:** after casting a sorcerer spell of 1st level or higher, the DM can have Wren roll on the Wild Magic Surge table (1-in-20 chance per cast). Effects are random, often chaotic, occasionally spectacular — this is the whole point of her.
- **Tides of Chaos:** once per long rest (until triggered again), gain advantage on one attack roll, ability check, or saving throw. Using this gives the DM license to force a Wild Magic Surge the next time she casts a sorcerer spell.
- **Bend Luck** (6th level): reaction, spend 2 sorcery points to roll 1d4 and apply it as a bonus or penalty to another creature's attack roll, ability check, or saving throw.
- **Controlled Chaos** (14th level): when rolling on the Wild Magic Surge table, can roll twice and choose which result occurs.

> **DM quick-check per sorcerer spell cast (1st level+): Wild Magic Surge uses a running counter, not flat 1-in-20 (house rule, Session 43 — fits her arc better). Surge threshold = 1 + consecutive non-surge casts since her last surge (0 misses = surge on a 1, 3 misses = surge on 1-4, etc.). Resets to 0 whenever a surge triggers, and also on a long rest. Track the live miss-count in Current Status below, update it every cast. A forced surge (Tides of Chaos debt unpaid) always triggers regardless of the counter, uses Controlled Chaos (roll twice, pick the better) since she's 14th+ level, and resets the counter same as any surge. Chaos Bolt chains to a new target if both d8s roll the same number — check every cast, not just on request. Misty Step moves only Wren herself; Dimension Door can bring one willing creature within 5 ft along with her.**

**Known Spells**
- **Cantrips (6):** Fire Bolt, Ray of Frost, Minor Illusion, Prestidigitation, Mage Hand, Light
- **1st (3):** Chaos Bolt, Shield, Mage Armor
- **2nd (3):** Misty Step, Scorching Ray, Mirror Image
- **3rd (3):** Fireball, Counterspell, Haste
- **4th (2):** Greater Invisibility, Dimension Door
- **5th (2):** Cone of Cold, Wall of Force
- **6th (1):** Chain Lightning
- **7th (1):** Prismatic Spray

*Chaos Bolt (1st level): ranged spell attack, 2d8+1d6 damage of a randomly determined type; on a hit, if the two d8s roll the same number, the spell leaps to a new target and can keep chaining — the single most "Wren" spell in her whole list.*

**Criminal (background)**
- **Criminal Contact:** a network of quiet, unofficial connections through Ravensmoor's fairground district — people who owe her favors or simply know how to not ask questions.

## Equipment
- A dagger, rarely drawn
- Simple traveling clothes, nothing that marks her as anything but ordinary — deliberate camouflage from a life spent trying not to be noticed
- A small, battered tin she keeps trinkets in — nothing valuable, just things she's held onto through years of moving on quickly
- No coin worth mentioning; owns almost nothing, by long habit rather than choice

## Personality
- **Traits:** Direct to the point of bluntness — has no patience left for people who dance around a question. Watches exits and hands before anything else in a room.
- **Ideals:** Choice. Nobody should have power — or lack it — without some say in the matter. That's what was taken from her, and what she means to make sure isn't taken from anyone else she can help.
- **Bonds:** Sael, who found her first and told her the truth before asking anything in return. The four guardians, briefly connected to at Ravensmoor — proof that whatever she's afraid of becoming, others have crossed that same ground and come out the other side.
- **Flaws:** Years of isolation trained her to expect betrayal or exploitation from anyone offering help. Trusts slowly, and still braces for the offer to turn out conditional.

## Backstory
Nobody in Ravensmoor's fairground district knew her by anything but "the Spark" — a name said half fondly, half nervously, for a young woman whose untrained magic kept surfacing in small, uncontrolled bursts: a fire that started and stopped itself, a wall that rebuilt itself crookedly overnight, a cart that floated gently across a flooded street. No formal training, no guild registration, no family or teacher to explain any of it. She learned early that power without choice in it isn't power at all — it's just a different kind of cage — and spent months living on the edges of the city, alone, afraid of hurting someone by accident again.

Sael found her in an alley near the fairground's edge, approached with the same patient stillness she'd once wished someone had shown her, and told her the truth about her own years of isolation before asking anything in return. That honesty — and later, a direct, unflinching conversation with Rurik about the difference between raw ability and actual power (choice, freedom) — was enough to bring her in. A brief connection to all four sentinel guardians, arranged so she could feel firsthand that others had survived exactly this kind of fear, sealed it. She joined the party at Ravensmoor, chosen freely rather than recruited, on the promise that Urskelde deals in chaos and unpredictability rather than trying to file her power down into something safer and smaller.

## Current Status
- Conditions: none — untouched in the Greyfeather ambush, road to Ravensmoor (Session 43)
- HP: 82 / 82 (full)
- **Wild Magic Surge counter (Session 43 house rule): 5 consecutive non-surge casts today** (Haste, quickened Chain Lightning, Cone of Cold, quickened Chaos Bolt, upcast Chaos Bolt) — next natural roll would surge on 1-6. Moot for the very next cast: a forced surge is already owed (see below), which triggers regardless and resets this counter to 0 afterward.
- Sorcery points: 9/15 (2 spent quickening Chain Lightning, 2 spent quickening Chaos Bolt, 2 spent on Bend Luck against the Handler's save)
- Spell slots (Session 43, full day): 3/4 1st (Chaos Bolt x2), 3/3 2nd, 2/3 3rd (Haste), 3/3 4th, 1/2 5th (Cone of Cold), 0/1 6th (Chain Lightning), 1/1 7th
- Tides of Chaos: **spent** (advantage on the upcast Chaos Bolt attack that killed the Handler) — **forced surge now owed on her next sorcerer spell cast**, will use Controlled Chaos (roll twice, pick the better) since she's 14th+ level
- **First real fight fully let loose (Session 42):** at Rurik's urging, dropped all restraint at the Cellarworks door — a thunderclap surge deafened the block on top of a full Fireball, exactly the chaos the plan called for. Rurik told her plainly afterward it was mastery, not luck — the first time anyone's framed her power that way. Days later, in the fight against the Auditor and the boarding action, no harmful surge landed either — three real fights running clean.
- **Joined the party (Session 35, Ravensmoor)** — recruited specifically for the gap in the party's arcane magic, ahead of an approach to the unscouted Tallow House. Connected briefly to Kethrun, Ilsevet, Vessyn, and Skarn as part of Rurik's welcome — the first time she's felt understood rather than feared.
- **Milestone level-up (Session 42, Halloway's Reach):** Sorcerer 15 → 16 — her first milestone since joining, earned alongside the rest of the party for securing Halloway's Reach and closing a third account (the freed courier) in front of witnesses. **Ability Score Improvement: +2 CON (14 → 16, reflected above)** — better concentration saves and survivability for a caster who leans hard into Wild Magic's unpredictability. No new Wild Magic feature at this level. Rolled +2 (d6) + CON mod 3, now **82 max**.
